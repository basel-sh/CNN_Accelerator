//==============================================================================
// tb_top_2px_mp4.v | Verification for the 4-DSP time-multiplexed accelerator
//==============================================================================
`timescale 1ns/1ps
module tb_top_2px_mp4;
    ...
    wire Out_Valid, Out_Full, Busy, Scan_Done, Fast_Locked;

    top_2px_mp4 #(
        .Pixel_W(Pixel_W), .Kernel_W(Kernel_W), .Acc_W(Acc_W),
        .Img_W(Img_W), .Img_H(Img_H), .K(K),
        .Img_Addrw(Img_Addrw), .Kidxw(Kidxw), .Bypass_Lock(1'b1)
    ) Dut (
        ...
        .Out_Valid(Out_Valid), .Out_Full(Out_Full), .Fast_Locked(Fast_Locked)
    );

    always #25 Clk = ~Clk; // 20 MHz system clock
    ...
    initial begin
        Relu_Mode = 0;
        if ($value$plusargs("RELU=%d", Relu_Mode))
            Relu_En = (Relu_Mode != 0);
        $display("TB_TOP_2PX_MP4: ReLU=%0d", Relu_En);

        $dumpfile("sim/tb_top_2px_mp4.vcd");
        $dumpvars(0, tb_top_2px_mp4);

        $readmemh("Images/input_32x32.mem", Image_Stim);
        $readmemh("Images/kernels/edge_3x3.mem", Kernel_Stim);

        Out_File = $fopen("sim/rtl_output_2px_mp4.mem", "w");
        Capture_Count = 0;
        Pair_Count = 0;
        ...
        repeat (Total_Pix + 250) @(posedge Clk);

        $display("TB_TOP_2PX_MP4: captured %0d outputs from %0d pairs (expected %0d outputs)",
                 Capture_Count, Pair_Count, Expected_Outputs);
        if (Capture_Count == Expected_Outputs)
            $display("TB_TOP_2PX_MP4: OUTPUT COUNT MATCHES EXPECTATION");
        else
            $display("TB_TOP_2PX_MP4: *** OUTPUT COUNT MISMATCH ***");
