//==============================================================================
// tb_generic.v — reusable single-frame system-level harness for top_2px.
// Geometry set via -P command-line overrides at compile time. Reads
// mem/image.mem + mem/kernel.mem, writes sim/rtl_output.mem, exactly like
// Testbench/tb_top_2px.v does, so Python/verify.py-style comparison works
// unchanged.
//==============================================================================
`timescale 1ns/1ps
module tb_generic;
    parameter Pixel_W=8, Kernel_W=8, Acc_W=20;
    parameter Img_W=32, Img_H=32, K=3;
    parameter Img_Addrw=10, Kidxw=4;
    parameter RELU=0;
    ...
    initial begin
        Relu_En = (RELU != 0);
        $display("TB_GENERIC: ReLU=%0d Img_W=%0d Img_H=%0d K=%0d", Relu_En, Img_W, Img_H, K);

        $readmemh("mem/image.mem", Image_Stim);
        $readmemh("mem/kernel.mem", Kernel_Stim);
        ...
        $display("TB_GENERIC: captured %0d outputs (expected %0d)", Capture_Count, Expected_Outputs);
        if (Capture_Count == Expected_Outputs)
            $display("TB_GENERIC: OUTPUT COUNT MATCHES EXPECTATION");
        else
            $display("TB_GENERIC: *** OUTPUT COUNT MISMATCH ***");
