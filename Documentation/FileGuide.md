...
its responsibility, what will later be implemented inside it, and which
files it interacts with. Nothing in the tree is unexplained.
...
| `expected_output.png` | Visual sanity check alongside the numeric PASS/FAIL | Golden-model output rendered as an image |
...
|---|---|---|
| `Project.xpr` | Vivado's own project file | Generated by `Scripts/build.tcl` — not authored by hand, not pre-created |
...
|---|---|---|---|
| `build.tcl` | Vivado project creation must be reproducible, not manual-only | Creates `Project.xpr`, adds RTL/testbench/constraints sources | `RTL/`, `Testbench/`, `Vivado/constraints.xdc` |
